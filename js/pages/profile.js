...
const ProfilePage = (() => {
  const render = () => {
    ...
    renderNotifications();
    renderSettings();
    renderLoyalty(customer);
  };

  // بطاقة نقاط الولاء — "قريبًا" لحد ما يتفعّل من سلطان ERP، وبعدين رقم حقيقي.
  // فشل الجلب (قبل ما الـmigration يتشغّل مثلاً) بيسيب البطاقة على "قريبًا"
  // بهدوء، مايكسرش الصفحة
  const renderLoyalty = async (customer) => {
    const el = document.getElementById('profile-loyalty');
    if (!el || !customer?.id) return;
    const settings = await API.getLoyaltySettings();
    if (!settings.enabled) {
      el.innerHTML = `
        <div class="loyalty-card soon">
          <div>
            <div class="lbl">🎁 نقاط الولاء</div>
            <div class="sub">قريبًا</div>
          </div>
          <div class="val">قريبًا</div>
        </div>`;
      return;
    }
    const balance = await API.getCustomerLoyalty(customer.id);
    el.innerHTML = `
      <div class="loyalty-card">
        <div>
          <div class="lbl">🎁 رصيد نقاطك</div>
          <div class="sub">تواصل معانا على واتساب لاستبدالها</div>
        </div>
        <div class="val">${balance.toFixed(0)}</div>
      </div>`;
  };
